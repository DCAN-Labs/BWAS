...
for i=1:length(this.data)
    ...
    switch this.data{i}.attributes.Encoding
        ...
        case 'ExternalFileBinary'
            ...
            this.data{i}.attributes.ExternalFileName   = [f e];
...
if isempty(this.label)
    ...
    for i=1:length(this.label.name)
        fprintf(fid,'%s<Label %s="%s" %s="%s" %s="%s" %s="%s" %s="%s"><![CDATA[%s]]></Label>\n',o(2),...
            this.label.index{i}.key{1},num2str(this.label.index{i}.val{1}),...
            this.label.index{i}.key{2},num2str(this.label.index{i}.val{2}),...
            this.label.index{i}.key{3},num2str(this.label.index{i}.val{3}),...
            this.label.index{i}.key{4},num2str(this.label.index{i}.val{4}),...
            this.label.index{i}.key{5},num2str(this.label.index{i}.val{5}),...
            this.label.name{i});
    end
...
%     end
    ...
    switch this.data{i}.attributes.Encoding
        ...
        case 'GZipBase64Binary'
            ...
            % uses native machine format
        case 'ExternalFileBinary'
            extfilename = fullfile(fileparts(fopen(fid)),[f e]);
            if ~def.offset
